...
def process_files(input_files: FileInput):
    try:
        ...
        log_error(ede, context="EmptyDataError in process_files: Possibly an empty input file.")
        raise
    except ValueError as ve:
        log_error(ve, context="ValueError in process_files: Likely due to unsupported file format or incorrect data processing.")
        raise
    except KeyError as ke:
        log_error(ke, context="KeyError in process_files: Missing expected columns in one of the data frames.")
        raise
    except Exception as e:
        log_error(e, context="General error in process_files.")
        raise